    ...
    try {
      ...
      if (result.exitCode === 0 || result.exitCode === 1) {
        try {
          ...
          // Only handoff INFO-level findings that indicate POTENTIAL for deeper testing.
          //
          // CONFIRMED (critical/high/medium) → Triage directly (no re-testing needed)
          // INFO-level (reflections, endpoints, tech detection) → Specialized agents for deeper testing

          // Separate confirmed vulnerabilities from info-level indicators
          ...
          // INFO-level findings that indicate potential for XSS testing (reflections, inputs)
          const xssIndicators = infoFindings.filter(
            (f) =>
              f.info?.tags?.includes('reflection') ||
              f.info?.tags?.includes('input') ||
              f.info?.name?.toLowerCase().includes('reflection') ||
              f['template-id']?.toLowerCase().includes('reflection')
          );

          // INFO-level findings that indicate potential for SQLi testing (error messages, db detection)
          const sqliIndicators = infoFindings.filter(
            (f) =>
              f.info?.tags?.includes('error') ||
              f.info?.name?.toLowerCase().includes('database') ||
              f.info?.name?.toLowerCase().includes('mysql') ||
              f.info?.name?.toLowerCase().includes('postgres')
          );

          // INFO-level findings for SSRF testing (URL parameters, redirects)
          const ssrfIndicators = infoFindings.filter(
            (f) =>
              f.info?.tags?.includes('redirect') ||
              f.info?.tags?.includes('url-parameter') ||
              f.info?.name?.toLowerCase().includes('redirect')
          );

          // Only handoff INFO-level indicators to specialized agents for deeper testing
          if (xssIndicators.length > 0) {
            await this.handoffToXSSAgent(job.id!, programId, xssIndicators, s3Key);
            logger.info(
              { count: xssIndicators.length },
              '📤 Handoff: INFO-level XSS indicators → XSS Agent for deep testing'
            );
          }

          if (sqliIndicators.length > 0) {
            await this.handoffToSQLiAgent(job.id!, programId, sqliIndicators, s3Key);
            logger.info(
              { count: sqliIndicators.length },
              '📤 Handoff: INFO-level SQLi indicators → SQLi Agent for deep testing'
            );
          }

          if (ssrfIndicators.length > 0) {
            await this.handoffToSSRFAgent(job.id!, programId, ssrfIndicators, s3Key);
            logger.info(
              { count: ssrfIndicators.length },
              '📤 Handoff: INFO-level SSRF indicators → SSRF Agent for deep testing'
            );
          }
          ...
          logger.info(
            {
              confirmedVulns: confirmedFindings.length,
              infoIndicators: infoFindings.length,
              xssIndicators: xssIndicators.length,
              sqliIndicators: sqliIndicators.length,
              ssrfIndicators: ssrfIndicators.length,
            },
            '🧠 Smart handoff: Confirmed vulns → Triage, Info indicators → Specialized agents'
          );

          // Handoff CONFIRMED findings to triage agent for AI analysis
