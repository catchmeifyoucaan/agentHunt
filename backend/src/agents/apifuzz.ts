    ...
    try {
      ...
      const highConfidenceAPI = findings.filter((f: any) => {
        ...
        return (
          ...
          (name.includes('auth') || name.includes('bypass') ||
           name.includes('injection') || name.includes('idor') ||
           name.includes('graphql') || name.includes('ssrf') ||
           tags.includes('auth-bypass') || tags.includes('idor') ||
  ...
  private async handoffToConfirm(
    ...
    const byType = {
      ...
      injection: vulnerabilities.filter(v => {
        ...
        return name.includes('injection') || name.includes('sqli') ||
               name.includes('nosql') || tags.includes('injection');
      }).length,
    ...
    const severityScore = vulnerabilities.reduce((acc, v) => {
    ...
    await this.createRichHandoff(apifuzzJobId, programId, 'confirm', {
      parentResult: {
        agentType: 'apifuzz',
        apiType,
        summary: {
      ...
      reasoning: {
        ...
        decisionFactors: [
          ...
          apiType === 'graphql' ? 'GraphQL-specific validation needed' : 'REST API confirmation required',
      ...
      objectives: {
        ...
        secondary: [
          ...
          apiType === 'graphql' ? 'Validate GraphQL introspection and mutation abuse' : 'Confirm REST parameter pollution',
      ...
      successCriteria: {
        ...
        requiredFields: ['url', 'method', 'vulnerability_type', 'confirmation_status', 'exploit_proof'],
