...
export class MultiAIProvider {
  ...
  async chat(
    ...
    options: {
      ...
      operationName?: string; // For better trace naming
    } = {}
  ): Promise<AIResponse> {
    const { temperature = 0.0, maxTokens = 4096, systemPrompt, preferredProvider, operationName = 'chat' } = options;
    ...
    // Use enhanced tracing service for LLM calls
    const llmSpan = tracingService.startLLMSpan(operationName, {
      ...
      try {
        ...
        for (const providerConfig of sortedProviders) {
          try {
            ...
            // Calculate cost
            const costUsd = this.calculateCost(result);

            // Complete LLM span with full response data for Phoenix
            tracingService.completeLLMSpan(llmSpan, {
            ...
            // Add success attributes to legacy span
            span.setAttributes({
              ...
              'ai.response_length': result.content?.length || 0,
            });
        ...
        llmSpan.setStatus({ code: SpanStatusCode.ERROR, message: errorSummary });
        llmSpan.end();
