...
class WorkflowTracingService {
  ...
  private async findRootJob(jobId: string): Promise<string | null> {
    ...
    while (currentId) {
      const result = await database.query('SELECT parent_job_id FROM jobs WHERE id = $1', [
        currentId,
      ]);
   ...
   */
  private async buildJobTraceTree(
    jobId: string,
    depth: number = 0,
  ...
  async getWorkflowStats(jobId: string): Promise<{
    ...
    const avgJobDuration =
      completedCount > 0 ? Math.round(totalJobDuration / completedCount / 1000) : 0;

    // Calculate total workflow duration
    const creationTimes = allJobs.map((j) => new Date(j.createdAt).getTime());
    const completionTimes = completedJobs
      .map((j) => (j.completedAt ? new Date(j.completedAt!).getTime() : null))
