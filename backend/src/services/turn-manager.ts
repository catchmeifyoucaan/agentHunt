...
export class TurnManager {
  ...
  /**
   * Ensure underlying tables exist and database connectivity works.
   * Throws if migrations have not been applied.
   */
  async healthCheck(): Promise<void> {
    try {
      const result = await database.query(
        `SELECT
           to_regclass('public.turns') as turns_table,
           to_regclass('public.interactions') as interactions_table,
           to_regclass('public.actions') as actions_table`
      );

      const tables = result.rows[0];
      if (!tables?.turns_table || !tables?.interactions_table || !tables?.actions_table) {
        throw new Error('TurnManager tables missing. Run migrations (turns/interactions/actions).');
      }

      logger.info('TurnManager health check passed');
    } catch (error: any) {
