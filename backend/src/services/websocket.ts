...
class WebSocketService {
  ...
  private async subscribeToRedis(): Promise<void> {
    ...
    // Subscribe to agent logs channel for Live Terminal
    await this.redisSubscriber.psubscribe('agent:*:log');
    await this.redisSubscriber.subscribe('agent:logs');
    ...
    this.redisSubscriber.on('pmessage', (pattern: string, channel: string, message: string) => {
      try {
        ...
        } else if (channel.includes(':log') || channel === 'agent:logs') {
          // Broadcast agent logs to all clients for Live Terminal
          this.broadcastToAll({
            type: 'log',
            ...data,
    ...
    // Also handle regular subscribe messages (non-pattern)
