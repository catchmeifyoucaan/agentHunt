...
export interface KnowledgeEntry {
  id: string;
  type: 'vulnerability' | 'exploit' | 'technique' | 'finding' | 'template' | 'tool' | 'strategy';
...
export interface StrategyKnowledge extends KnowledgeEntry {
  type: 'strategy';

  // Strategy details
  trigger: string; // What triggers this strategy
  confidence: number; // Initial confidence score
  steps: Array<{
    agent: string;
    action: string;
    params?: any;
  ...
  // Relationships
  prerequisites: string[]; // IDs of prerequisite knowledge/conditions
  relatedTechniques: string[]; // IDs of related techniques
  
  // Evolution
  successes: number;
  failures: number;
  lastSuccessfulContext?: any;
}
