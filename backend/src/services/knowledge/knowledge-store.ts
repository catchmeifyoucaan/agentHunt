...
import strategySynthesizer from './strategy-synthesizer';
...
class KnowledgeStore {
  ...
  async storeFeedback(feedback: AgentFeedback): Promise<void> {
    try {
      ...
      // 🚀 STRATEGY SYNTHESIS: Analyze feedback to learn new attack strategies
      // This enables the system to auto-synthesize complex attack chains from successful patterns
      try {
        await strategySynthesizer.analyzeFeedback(feedback);
      } catch (error) {
        logger.error({ error, feedbackId: feedback.id }, 'Strategy synthesis failed (non-fatal)');
      }
    } catch (error: any) {
