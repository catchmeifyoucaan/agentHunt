...
class RichHandoffService {
  ...
  /**
   * Reject a handoff with reason, with optional retry/escalation
   */
  async rejectHandoff(
    handoffId: string,
    reason: string,
    options?: { retry?: boolean; escalate?: boolean }
  ): Promise<void> {
    try {
      const handoff = await this.getHandoff(handoffId);

      // Handle retry logic
      if (options?.retry && handoff) {
        const retryCount = (handoff.context as any)._retryCount || 0;
        const maxRetries = 3;

        if (retryCount < maxRetries) {
          logger.info(
            { handoffId, retryCount, maxRetries },
            'Retrying handoff after rejection'
          );

          // Create a new handoff with incremented retry count
          const newContext = {
            ...handoff.context,
            _retryCount: retryCount + 1,
            _previousRejection: reason,
          };

          await this.createHandoff(
            handoff.fromAgent,
            handoff.toAgent.type,
            newContext as any,
      ...
      if (options?.escalate && handoff) {
        logger.warn(
          { handoffId, originalTarget: handoff.toAgent.type },
          'Escalating handoff to manager agent'
        );

        // Create escalation handoff to manager
        const escalationContext = {
          ...handoff.context,
          _escalatedFrom: handoff.toAgent.type,
