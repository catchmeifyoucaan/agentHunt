...
import { ClickjackingAgent } from '../agents/clickjacking';
import { DNSRebindingAgent } from '../agents/dns-rebinding';
import { CRLFInjectionAgent } from '../agents/crlf-injection';
import { OpenRedirectAgent } from '../agents/open-redirect';
import { LFIRFIAgent } from '../agents/lfi-rfi';
import { NoSQLInjectionAgent } from '../agents/nosql-injection';
import { HostHeaderInjectionAgent } from '../agents/host-header-injection';
import { CSVInjectionAgent } from '../agents/csv-injection';
import { LDAPInjectionAgent } from '../agents/ldap-injection';
import { SAMLInjectionAgent } from '../agents/saml-injection';
import { WebCacheDeceptionAgent } from '../agents/web-cache-deception';
import { MassAssignmentAgent } from '../agents/mass-assignment';
import { HTTPParameterPollutionAgent } from '../agents/http-parameter-pollution';
import { LaTeXInjectionAgent } from '../agents/latex-injection';
import { XPathInjectionAgent } from '../agents/xpath-injection';
import { CommandInjectionAgent } from '../agents/command-injection';
...
async function startWorkers() {
  ...
  try {
    await turnManager.healthCheck();
    logger.info('TurnManager ready');
  } catch (error: any) {
    logger.error({ error }, 'TurnManager health check failed');
    process.exit(1);
  }

  try {
    // We only need to instantiate once to ensure built-in patterns register.
    patternManager.listPatterns();
    logger.info('PatternManager ready with %d patterns', patternManager.listPatterns().length);
  } catch (error: any) {
    logger.error({ error }, 'PatternManager initialization failed');
    process.exit(1);
  }
  ...
  // PayloadsAllTheThings agents
  const clickjackingAgent = new ClickjackingAgent();
  const dnsRebindingAgent = new DNSRebindingAgent();
  const crlfInjectionAgent = new CRLFInjectionAgent();
  const openRedirectAgent = new OpenRedirectAgent();
  const lfiRfiAgent = new LFIRFIAgent();
  const nosqlInjectionAgent = new NoSQLInjectionAgent();
  const hostHeaderInjectionAgent = new HostHeaderInjectionAgent();
  const csvInjectionAgent = new CSVInjectionAgent();
  const ldapInjectionAgent = new LDAPInjectionAgent();
  const samlInjectionAgent = new SAMLInjectionAgent();
  const webCacheDeceptionAgent = new WebCacheDeceptionAgent();
  const massAssignmentAgent = new MassAssignmentAgent();
  const httpParameterPollutionAgent = new HTTPParameterPollutionAgent();
  const latexInjectionAgent = new LaTeXInjectionAgent();
  const xpathInjectionAgent = new XPathInjectionAgent();
  const commandInjectionAgent = new CommandInjectionAgent();
  const fileUploadAgent = new FileUploadAgent();
  ...
  const agentMap = new Map<AgentType, { instance: any; concurrency: number }>([
    ...
    // PayloadsAllTheThings agents
    ['clickjacking', { instance: clickjackingAgent, concurrency: 25 }],
    ['dns-rebinding', { instance: dnsRebindingAgent, concurrency: 15 }],
    ['crlf-injection', { instance: crlfInjectionAgent, concurrency: 30 }],
    ['open-redirect', { instance: openRedirectAgent, concurrency: 40 }],
    ['lfirfi', { instance: lfiRfiAgent, concurrency: 30 }],
    ['nosql-injection', { instance: nosqlInjectionAgent, concurrency: 25 }],
    ['host-header-injection', { instance: hostHeaderInjectionAgent, concurrency: 30 }],
    ['csv-injection', { instance: csvInjectionAgent, concurrency: 20 }],
    ['ldap-injection', { instance: ldapInjectionAgent, concurrency: 20 }],
    ['saml-injection', { instance: samlInjectionAgent, concurrency: 15 }],
    ['web-cache-deception', { instance: webCacheDeceptionAgent, concurrency: 25 }],
    ['mass-assignment', { instance: massAssignmentAgent, concurrency: 30 }],
    ['http-parameter-pollution', { instance: httpParameterPollutionAgent, concurrency: 30 }],
    ['latex-injection', { instance: latexInjectionAgent, concurrency: 15 }],
    ['xpath-injection', { instance: xpathInjectionAgent, concurrency: 25 }],
    ['command-injection', { instance: commandInjectionAgent, concurrency: 20 }],
    ['file-upload', { instance: fileUploadAgent, concurrency: 25 }],
