...
/**
 * Helper: Extract main domain/company name from parsed scope for program naming
 * Examples:
 *   - "api.spotify.com" -> "Spotify"
 *   - "mail.google.com" -> "Google"
 *   - "sub.example.co.uk" -> "Example"
 */
function extractProgramName(parsedScope: any): string {
  // Collect all domains and subdomains
  const allDomains: string[] = [
    ...(parsedScope.domains || []),
    ...(parsedScope.subdomains || []),
    ...(parsedScope.wildcardDomains || []),
  ];

  if (allDomains.length === 0) {
    // Try to extract from URLs
    const urls = parsedScope.urls || [];
  ...
  // Find the most common root domain
  const rootDomainCounts: Record<string, number> = {};

  for (const domain of allDomains) {
    const rootDomain = extractRootDomain(domain);
    if (rootDomain) {
      rootDomainCounts[rootDomain] = (rootDomainCounts[rootDomain] || 0) + 1;
    }
  }

  // Get the most common root domain
  let mostCommonDomain = '';
  let maxCount = 0;

  for (const [domain, count] of Object.entries(rootDomainCounts)) {
    if (count > maxCount) {
      maxCount = count;
  ...
  // Extract company name from root domain (e.g., "spotify.com" -> "Spotify")
  const companyName = extractCompanyName(mostCommonDomain);
  return companyName;
}

/**
 * Extract root domain from a full domain/subdomain
 * Examples:
...
function extractRootDomain(domain: string): string {
  ...
  let cleaned = domain.replace(/^https?:\/\//, '').replace(/\/.*$/, '').toLowerCase();
  ...
  if (multiPartTLDs.includes(lastTwo) && parts.length >= 3) {
    // e.g., "example.co.uk" -> return "example.co.uk"
    return parts.slice(-3).join('.');
  }

  if (parts.length >= 2) {
    // e.g., "spotify.com" -> return "spotify.com"
...
/**
 * Extract company/brand name from root domain and format it nicely
 * Examples:
 *   - "spotify.com" -> "Spotify"
 *   - "google.co.uk" -> "Google"
 *   - "hack-the-box.com" -> "Hack The Box"
 */
function extractCompanyName(rootDomain: string): string {
  // Get the main part (before the TLD)
  const parts = rootDomain.split('.');
  let companyPart = parts[0];

  // Handle multi-part TLDs
  const multiPartTLDs = ['co', 'com', 'org', 'net', 'gov', 'ac', 'edu'];
  if (parts.length >= 3 && multiPartTLDs.includes(parts[parts.length - 2])) {
  ...
  if (uppercaseWords[lowerCompany]) {
    return uppercaseWords[lowerCompany];
  }

  return formatted || `Program-${Date.now()}`;
}
...
router.post('/scope', multerMiddleware, async (req, res) => {
  try {
    ...
    if (program_id) {
      ...
      // Auto-generate program name from domains if not provided
      const autoGeneratedName = program_name || extractProgramName(parsedScope);
      ...
      logger.info(
        { programName: autoGeneratedName, autoGenerated: !program_name },
        'Program name determined'
      );
    } else {
