...
import { Settings, Bell, Shield, Zap, Database, CheckCircle, AlertCircle, Brain, GitBranch, Server } from 'lucide-react';
import { settingsApi } from '@/lib/api';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { Badge } from '@/components/ui/badge';
import { getAllAgents } from '@/lib/agentMetadata';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch('/api/v1/routing/rules')
      .then(res => res.json())
      .then(data => setRoutingRules(data.rules || []))
      .catch(console.error);
  }, []);
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-semibold">Dynamic Routing Rules</h2>
          </div>
          <div className="space-y-4">
            {routingRules.length === 0 ? (
              <p className="text-muted-foreground text-sm">Loading routing rules...</p>
            ) : (
              <div className="grid gap-4">
                {routingRules.map((rule, idx) => (
                  <div key={idx} className="border rounded-md p-4 bg-slate-50 dark:bg-slate-900">
                    ...
                    <div className="space-y-2">
                      {rule.targets.map((target: any, tIdx: number) => (
                        <div key={tIdx} className="flex items-center gap-3 text-sm">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {target.agents.map((agent: string) => (
                          ...
                          {target.hasCondition && (
                            <span className="text-xs text-orange-600 italic flex items-center gap-1">
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-semibold">Agent Fleet Configuration</h2>
          </div>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {agents.map((agent) => (
                <div key={agent.type} className="flex items-start justify-between p-3 border rounded-md">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-md ${agent.color.replace('bg-', 'bg-opacity-20 text-').replace('500', '600')}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={agent.category === 'exploitation' ? 'destructive' : 'secondary'}>
                      {agent.category}
                    </Badge>
                  </div>
                </div>
