...
import { Terminal, CheckCircle, XCircle, Zap } from 'lucide-react';
...
export default function ManagerAgentPage() {
  ...
  const executeCommand = async (cmd: string) => {
    ...
    try {
      const response = await fetch('/api/v1/manager/command', {
        ...
        body: JSON.stringify({ command: cmd, user_id: 'frontend-user' }),
      });
  ...
  const quickActions = [
    { label: 'System Status', command: 'system status', danger: false },
    { label: 'Health Report', command: 'generate health report', danger: false },
    { label: 'Monitor Queues', command: 'monitor queues', danger: false },
    { label: 'Pause Queues', command: 'pause all queues', danger: true, warning: '⚠️ This will PAUSE ALL agent queues. Are you sure?' },
    { label: 'Resume Queues', command: 'resume all queues', danger: false },
    { label: 'Flush Redis', command: 'flush redis', danger: true, warning: '🔥 This will FLUSH ALL Redis data (cache & memory). This is DESTRUCTIVE. Confirm?' },
    { label: 'Flush Database', command: 'flush database', danger: true, warning: '💀 This will DELETE ALL FINDINGS from the database. This is IRREVERSIBLE. Confirm?' },
    { label: 'Kill All Jobs', command: 'kill all jobs', danger: true, warning: '🚫 This will KILL ALL running jobs immediately. Confirm?' },
  ];

  const handleQuickAction = (action: typeof quickActions[0]) => {
    if (action.danger && action.warning) {
      if (!window.confirm(action.warning)) {
        return;
      }
    }
      ...
      <Card className="flex-shrink-0 mb-4">
        <CardHeader>
          <CardTitle className="text-lg font-semibold flex items-center">
            <Zap className="w-5 h-5 mr-2" /> Quick Actions
          </CardTitle>
        </CardHeader>
        <CardContent>
